...
exports.quotaDashboardMarkdown = quotaDashboardMarkdown;
const historyStore_1 = require("./historyStore");
function fmt(n, unit) {
    if (unit === "acu")
        return `${Math.round(n * 10) / 10} ACU`;
    if (n >= 1e6)
        return `${(n / 1e6).toFixed(1)}M tokens`;
    if (n >= 1e3)
        return `${Math.round(n / 1e3)}k tokens`;
    return `${Math.round(n)} tokens`;
}
function quotaDashboardMarkdown(snapshots, executions, now = new Date()) {
    const lines = [];
    ...
    lines.push("");
    const consumption = (0, historyStore_1.consumptionByEngine)(executions);
    ...
    lines.push("");
    const calib = (0, historyStore_1.calibrationBySize)(executions);
...
//# sourceMappingURL=quotaDashboard.js.map