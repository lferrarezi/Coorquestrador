...
class ChatPanelProvider {
    ...
    saveSelection() { void this.deps.state.update(SELECTION_KEY, this.selection); }
    setEngines(snaps) {
        this.engines = snaps;
    ...
    resolveWebviewView(view) {
        ...
        view.webview.onDidReceiveMessage(async (msg) => {
            switch (msg.type) {
                ...
                case "cancelTask":
                    this.draft = null;
                    this.post({ type: "info", text: "Tarefa descartada." });
                    break;
            }
        });
    }
    ...
    async detect() {
        ...
        try {
            ...
            if (!this.selection.engineId || !installed.find((e) => e.id === this.selection.engineId)) {
                ...
                if (pref) {
                    this.selection = { engineId: pref.id, model: pref.default_model, power: "normal" };
                    this.saveSelection();
                }
            }
            this.post({ type: "installed", engines: installed, selection: this.selection });
            try {
                conf0.ensureAgentPacks();
                this.post({ type: "pack", name: conf0.activePack(), packs: conf0.listPacks() });
            }
            catch { /* */ }
    ...
    async handleChat(text) {
        ...
        if (!engineCfg) {
            this.post({ type: "error", text: `Assistente '${engineId}' indisponivel. Escolha um assistente detectado.` });
            return;
        }
        ...
        const prompt = (0, chat_1.buildChatPrompt)(system, this.history);
        const timeout = ef.defaults?.exec_timeout_seconds || 600;
        try {
            const full = await (0, chat_1.runChat)(engineForChat, prompt, c.root, timeout, power, (chunk) => this.post({ type: "assistantChunk", text: chunk }));
    ...
    async planDraft() {
        if (!this.draft) {
            this.post({ type: "error", text: "Nenhuma tarefa em edicao." });
            return;
        }
        const c = this.deps.cfg();
        const conf = this.configOrNull() || new config_1.CoorqConfig(c.root, c.configDir);
    ...
    routeEstimateAndPresent(demand, ef, cost, snaps, store, conf, note) {
        ...
        const quota = (0, estimator_1.estimateDemandQuota)(pending, cost);
        const est = (0, estimator_1.estimateDemand)(pending, cost);
        pending.forEach((t) => {
    ...
    async replanFailed() {
        if (!this.draft) {
            this.post({ type: "error", text: "Nenhuma tarefa em recuperacao." });
            return;
        }
        const c = this.deps.cfg();
        const conf = this.configOrNull() || new config_1.CoorqConfig(c.root, c.configDir);
        ...
        const store = new demandStore_1.DemandStore(conf.statePath());
        const demand = this.draft;
        const input = (0, planner_1.splitForReplan)(demand);
        if (input.failed.length === 0) {
            this.post({ type: "info", text: "Nada a recuperar: nenhuma etapa falhou." });
            return;
        }
        this.post({ type: "planStart" });
        try {
            ...
            conf.ensureAgentPacks();
            const agentSpec = fs.existsSync(conf.agentPath()) ? fs.readFileSync(conf.agentPath(), "utf8") : "(agente coorquestrador)";
            const prompt = (0, planner_1.buildReplanPrompt)({
                agentSpec, skills: conf.loadSkills(), input,
                projectContext: this.projectContext(c.root), snapshot: snaps, enginesMeta,
            });
            const plannerCfg = ef.engines[this.selection.engineId || c.plannerEngine] || ef.engines[c.plannerEngine];
            const raw = await (0, planner_1.runPlanner)(plannerCfg, prompt, c.root, ef.defaults.exec_timeout_seconds);
            const replanned = (0, planner_1.parsePlan)(raw);
            if (replanned.length === 0) {
                this.post({ type: "error", text: "O planejador nao retornou um plano de recuperacao valido." });
                return;
            }
            (0, planner_1.mergeReplanned)(demand, replanned);
            this.routeEstimateAndPresent(demand, ef, cost, snaps, store, conf, `Plano de recuperacao: ${input.completed.length} etapa(s) concluida(s) preservada(s), ${replanned.length} nova(s) etapa(s).`);
        }
        catch (e) {
    ...
    async execute() {
        if (!this.draft) {
            this.post({ type: "error", text: "Nenhuma tarefa para executar." });
            return;
        }
        const c = this.deps.cfg();
        const conf = this.configOrNull() || new config_1.CoorqConfig(c.root, c.configDir);
...
ChatPanelProvider.viewType = "coorq.chat";
