...
function jsonPathValue(obj: any, expr: string): any {
...
function lastJsonObject(stdout: string): any | null {
  ...
  for (const line of lines) {
    ...
    if (l.startsWith("{") && l.endsWith("}")) {
      try { return JSON.parse(l); } catch { /* segue */ }
    }
  }
  return null;
}
 ...
 */
function parseWithDefaults(stdout: string, unit: "token" | "acu"): number | null {
  ...
  if (obj && typeof obj === "object") {
    ...
    if (typeof obj.total_tokens === "number" && obj.total_tokens > 0) return obj.total_tokens;
  }
 ...
 */
export function measureUsage(cfg: EngineConfig | undefined, stdout: string): MeasuredUsage | null {
